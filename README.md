...
`ft_harvest_total.py (ex2)`: Requests three harvest weights and prints their total.

`ft_plant_age.py (ex3)`: Checks if a plant is ready to harvest based on its age in days.

`ft_water_reminder.py (ex4)`: Determines whether plants need watering based on days since last watering.

`ft_count_harvest_iterative.py (ex5)`: Counts from 1 to a given number using a loop, then prints “Harvest time!”.

`ft_count_harvest_recursive.py (ex5)`: Same as the iterative version, but implemented with recursion.
